...
class VoxelGrid(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self.num_voxels_base = num_voxels_base  #空间中一共有多少个base体素
        self.voxel_size_base = ((self.xyz_max - self.xyz_min).prod() / self.num_voxels_base).pow(1/3)  #每个base体素对应的实际空间中的边长

        # determine the density bias shift
        self.alpha_init = alpha_init            # set the alpha values everywhere at the begin of training
        ...
        self.density = torch.nn.Parameter(torch.zeros([1, 1, *self.world_size]))  #体素密度场 待优化的核心
    ...
    def _set_grid_resolution(self, num_voxels):
        ...
        self.num_voxels = num_voxels  # num_voxels：体素的总个数
        self.voxel_size = ((self.xyz_max - self.xyz_min).prod() / num_voxels).pow(1/3) #voxel_size：每个体素的实际边长 = [(总体积 / 体素个数)** 1/3]
        self.world_size = ((self.xyz_max - self.xyz_min) / self.voxel_size).long() # 实际的体素分辨率(xyz方向上各有多少个体素) (107,107,88) long() 用于取整
        self.voxel_size_ratio = self.voxel_size / self.voxel_size_base  # voxel_size_base :每个基体素的边长 voxel_size：实际体素的边长
        print('dvgo: voxel_size      ', self.voxel_size)
        print('dvgo: world_size      ', self.world_size)
        print('dvgo: voxel_size_base ', self.voxel_size_base)
        print('dvgo: voxel_size_ratio', self.voxel_size_ratio) # 实际的单位体素边长 / 基体素单位边长
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, is_train=False, **render_kwargs):
        ...
        N_samples = int(np.linalg.norm(np.array(self.density.shape[2:])+1) / stepsize) + 1  # 每一条光线最多被分为354个均匀小段 (stepsize = 0.5) 小段长度为0.5倍的单位体素长度
        # 2. determine the two end-points of ray bbox intersection
        vec = torch.where(rays_d==0, torch.full_like(rays_d, 1e-6), rays_d)  # 重要！！防止出现rays-d=0 进而出现Nan问题 后面需要除以vec
        rate_a = (self.xyz_max - rays_o) / vec
        rate_b = (self.xyz_min - rays_o) / vec
        t_min = torch.minimum(rate_a, rate_b).amax(-1).clamp(min=near, max=far)
        t_max = torch.maximum(rate_a, rate_b).amin(-1).clamp(min=near, max=far)
        # 3. check wheter a ray intersect the bbox or not
        mask_outbbox = (t_max <= t_min)
        # 4. sample points on each ray
        rng = torch.arange(N_samples)[None].float()
        if is_train:
            rng = rng.repeat(rays_d.shape[-2],1)  # 8192条光线,每条光线上有354段,对于每一条光线，加上一个【0,1】的随机数
            rng += torch.rand_like(rng[:,[0]])
        step = stepsize * self.voxel_size * rng  # 0.5 * 单位体素的实际边torch.rand_like长 * rng
        interpx = (t_min[...,None] + step/rays_d.norm(dim=-1,keepdim=True))
        rays_pts = rays_o[...,None,:] + rays_d[...,None,:] * interpx[...,None] #每条光线上的每个点的坐标
        # 5. update mask for query points outside bbox
        mask_outbbox = mask_outbbox[...,None] | ((self.xyz_min>rays_pts) | (rays_pts>self.xyz_max)).any(dim=-1)  #删除掉一些不在bbox内的点, 对于不相交的光线则整条删除
...
class MaskCache(nn.Module):
    ...
    def forward(self, xyz):
        ...
        if self.nearest:
            ...
            alpha = 1 - torch.exp(-F.softplus(density.to(self.voxel_size_ratio.device) + self.act_shift) * self.voxel_size_ratio)
            #alpha = 1 - torch.exp(-F.softplus(density+ self.act_shift) * self.voxel_size_ratio)
